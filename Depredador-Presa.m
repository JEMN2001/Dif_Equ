clear all
f=@(V) [V(1)-0.5*V(1)*V(2),-0.75*V(2)+0.25*V(1)*V(2)]; %Defino el campo vectorial del problema depredador presa,
...
b=20; %quiero llegar hasta t=20, esto lo pueden modificar ustedes
h=0.1; %quiero dar saltos de 0.1, esto lo pueden modificar ustedes
x0=3; %el valor inicial de x es 3, es decir x(t0)=x(0)=3
y0=4; %el valor inicial de y es 4, es decir y(t0)=y(0)=4
...
for i=1:length(t) %Recorro todas las entradas del vector que cree ahorita
    zf = Runge_Kutta_vec(f,[a,t(i)],[x0,y0],h); %Calculo cuanto es X y Y evaluados en el valor de la iteracón t(i)
...
plot(x,y); %Teniendo X y Y evaluados en todos los puntos de t (lo que hizo el for) ploteo Y con respecto a X
